...
def softmax(X):
    ...
    maxs = np.max(X.T, axis=0)
    diff_mat = (X.T-maxs).T
    exp_mat = np.exp(diff_mat)
    sum_mat = np.sum(exp_mat.T, axis = 0)
...
class SoftmaxClassifier():
    ...
    def cost_grad(self, X, y, W):
        ...
        cost = -np.sum([Yk[i].T.dot(np.log(softmax(np.dot(X[i],W)))) for i in range(input_size)])
        grad = -np.transpose(X).dot(Yk - softmax(X.dot(W)))
